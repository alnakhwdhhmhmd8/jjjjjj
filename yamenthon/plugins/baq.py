...
#
# This file is a part of < https://github.com/RepthonArabic/yamenthon/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/RepthonArabic/yamenthon/blob/master/LICENSE/>.
"""سـورس جـاكثون
So تخمـط الملـف اهينك واطشك للناس خماط واوثق عليك
Copyright (C) 2023 RepthonArabic. All Rights Reserved
Credit: https://github.com/RepthonArabic/yamenthon
Devloper: https://telegram.dog/T_A_Tl"""
"""import json
import math
import asyncio
import os
...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:  # Code by T.me/T_A_Tl
        taibaid = Config.OWNER_ID
    elif query_user_id == user_id or query_user_id == int(user_id):
        taibaid = user_id
    if query_user_id == Config.OWNER_ID or query_user_id == user_id or query_user_id == int(user_id) or query_user_id in Config.SUDO_USERS:  # Code by T.me/T_A_Tl
        ...
        if match2:
            ...
            for user in users:
                ...
                baqqir += " "
            baqqir = baqqir[:-1]
            old_msg = os.path.join("./yamenthon", f"{scc}.txt")
            try:
