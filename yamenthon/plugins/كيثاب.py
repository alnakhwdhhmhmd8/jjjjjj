...
@zq_lo.rep_cmd(
    ...
    info={
        "header": "Source code link of YamenThon",
        "usage": [
            "{tr}repo",
        ],
    },
...
async def source(e):
    ...
    await edit_or_reply(
        e,
        "Click [here](https://t.me/xvwwvl) to open this bot source code\
        \nClick [here](https://t.me/xvwwvl/117) to open supported link for heroku",
    )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(URL) as request:
            if request.status == 404:
                return await edit_delete(event, f"`{username} غيـر موجـود`")
            catevent = await edit_or_reply(event, "**- جـاري البحـث**")
            ...
            if sec_res.status_code == 200:
                ...
                for repo in sec_res.json():
                    ...
                    if limit == 0:
                        break
            REPLY = "**- معلومـات حسـاب كيثـاب** `{username}`\
                \n👤 **الاسـم:** [{name}]({html_url})\
                \n🔧 **النـوع:** `{type}`\
                \n🏢 **الشـركه:** `{company}`\
                \n🔭 **Blog** : {blog}\
                \n📍 **المـوقع** : `{location}`\
                \n📝 **البايـو** : __{bio}__\
                \n❤️ **المتـابعيين** : `{followers}`\
                \n👁 **يتـابـع** : `{following}`\
                \n📊 **المستـودع العـام** : `{public_repos}`\
                \n📄 **الجيسـت العـام** : `{public_gists}`\
                \n🔗 **انشـاء الحسـاب** : `{created_at}`\
                \n✏️ **تحديـث الحسـاب** : `{updated_at}`".format(
                username=username, **result
            )

            if repos:
                REPLY += "\n🔍 **بعـض المستـودعـات** : " + " | ".join(repos)
